import { updateOnboardingLookingForAction } from "@/lib/profile/actions";
import { getCurrentProfile } from "@/lib/profile/data";
import { SubmitButton } from "@/components/ui/SubmitButton";
import { TopBar } from "@/components/ui/TopBar";
import { lookingForChips } from "@/lib/mock-data";

type OnboardingLookingForPageProps = {
  searchParams: Promise<{
    error?: string;
  }>;
};

export default async function OnboardingLookingForPage({
  searchParams,
}: OnboardingLookingForPageProps) {
  const [{ error }, profile] = await Promise.all([searchParams, getCurrentProfile()]);
  const selected = new Set(profile.looking_for);
  ...
  return (
    <main>
      ...
      />
      <form action={updateOnboardingLookingForAction} className="wire-panel">
        {error ? (
          ...
          {lookingForChips.map((chip) => (
            <label key={chip}>
              <input
                ...
                name="lookingFor"
                value={chip}
                defaultChecked={selected.has(chip)}
